...
sys.path.insert(0,str(UP));sys.path.insert(0,str(UP/'emulate'))
from cpu import CPU8051
from memory import Memory
from hardware import create_hardware_hooks
class CpuBackend:
    name='cpu'
    def __init__(self,board): self.board=board;self.instructions=0
    def reset(self,firmware_size):
        b=self.board
        if not b.reset_asserted: b.assert_reset()
        self.memory=Memory();self.memory.reset()
        self.hw=b.reset_controller_model()
        ...
        b.release_reset()
        # Model the same cold flash-to-CODE copy convention as upstream emu.py.
        # This is not a claim to emulate undocumented silicon boot ROM.
        ...
        b.emit('CODE_LOAD',address=0,length=firmware_size,convention='upstream cold-load; boot ROM unmodeled')
    def _read(self,old):
        def read(a):
    ...
    def run(self,limit=200000,stop=b'BOOT=OK\n'):
        for _ in range(limit):
            ...
            cycles=self.cpu.step();self.instructions+=1;self.board.advance(max(cycles,1))
