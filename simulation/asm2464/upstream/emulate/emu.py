...
# Direct script execution puts only emulate/ on sys.path. Add the repository
# root as well because hardware models share codecs from tools/.
EMULATE_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = EMULATE_DIR.parent
sys.path.insert(0, str(EMULATE_DIR))
...
class Emulator:
    ...
    def load_firmware_from_spi(self, firmware_size: int, offset: int = 0x100):
        """Cold-load CODE memory from the preserved full SPI image.

        This models power-on boot, not CC31. The caller must supply the reviewed
        payload size because erased 0xFF bytes are valid firmware data and must
        not be guessed or stripped.
        """
        if firmware_size <= 0:
            raise ValueError("firmware_size must be positive")
        if offset < 0 or offset + firmware_size > len(self.hw.spi_flash):
            raise ValueError("firmware payload exceeds simulated SPI flash")
        if firmware_size > len(self.memory.code):
            raise ValueError("firmware payload exceeds emulated CODE capacity")
        self.memory.code[:] = b'\xFF' * len(self.memory.code)
        self.memory.load_firmware(
            bytes(self.hw.spi_flash[offset:offset + firmware_size]))
        self.hw.load_config_descriptor_from_rom()
        self.reset()

    def reset(self):
        """Reset emulator to initial state."""
        self.hw.reset_stock_msc_generation()
        self.memory.reset()
        self.cpu.reset()
        self.inst_count = 0
    ...
    def start_usb_device(self, driver: str = "dummy_udc", device: str = "dummy_udc.0"):
        ...
        This makes the emulator appear as a real USB device (VID:PID 1209:0001)
        ...
        try:
            ...
            print(f"[USB] Device should appear as 1209:0001 in lsusb")
...
"""
    ...
    parser.add_argument('--vidpid', type=str, default=None,
                        help='Override USB VID:PID in descriptor (e.g. 1209:0001)')
