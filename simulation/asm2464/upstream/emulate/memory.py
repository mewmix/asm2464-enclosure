...
class Memory:
    ...
    # Local HardwareState endpoint for DPX-banked XDATA.  Bank-one accesses
    # are MMIO even when their 16-bit address overlaps ordinary bank-zero
    # RAM, so they cannot rely on the sparse bank-zero hook ranges.
    dpx_hardware: object = field(default=None, repr=False)
    ...
    def read_xdata(self, addr: int) -> int:
        ...
        if dpx and self.proxy is not None:
            ...
            return value
        if dpx and self.dpx_hardware is not None:
            return self.dpx_hardware.read(addr)
    ...
    def write_xdata(self, addr: int, value: int):
        ...
        if dpx and self.proxy is not None:
            ...
            return
        if dpx and self.dpx_hardware is not None:
            self.dpx_hardware.write(addr, value)
            return
