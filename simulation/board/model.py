...
class Board:
    def __init__(self,config=None):
        self.config=config or load(); self.events=[];self.tick=0
    ...
    def reset_controller_model(self):
        # The upstream controller and its synthetic PCIe/NVMe target are coupled
        # internally. Keep their shared instance owned here, outside either CPU adapter.
        from hardware import HardwareState
        self.controller = HardwareState(log_reads=False, log_writes=False, log_uart=False)
        self.downstream = self.controller
    ...
    def assert_reset(self): self.reset_asserted=True;self.uart.clear();self.emit('RESET_ASSERT')
    ...
    def spi(self,master,opcode,address=0,data=b'',length=0):
        ...
        self.emit('SPI_TX',master=master,opcode=opcode,address=address,data=data.hex(),read_length=length)
        try:
            result=self.flash.transaction(opcode,address,data,length)
            self.emit('SPI_RX',master=master,data=result.hex());return result
    ...
    def brownout(self):
        self.assert_reset();self.flash.power(False);self.emit('POWER_OFF')
